...
- **`restartGame()`** (`js/game.js`) resets every global by hand and rebuilds `grid`. A new global that isn't reset there leaks across a restart. Three things are *intentionally* not reset: `hasLostOnce` (unlocks the speed control), `volMusic`/`volSFX` (which also outlive the session — see saved settings above), and the `gV`/`gD` grass noise.
...
**Saved settings.** `loadPrefs()`/`savePrefs()` (`js/state.js`) keep the two volumes in `localStorage` under `spiritgrove.prefs.v1`; `loadPrefs()` runs before the first `SND.bgm.volume` write, so the restored value is in place from the first frame. To persist another setting, add it to both functions and bump the key's version suffix if the shape changes incompatibly. Three rules the existing code follows and new fields must too: every `localStorage` access is inside `try/catch` (the property itself throws in a private window or with site data blocked, so a bare read crashes startup), the parsed object is **validated, not trusted** (`num01` clamps and rejects non-numbers — a hand-edited entry must not be able to break audio), and a failure to load or save is silent and non-fatal. Writes are bound to the slider's `change` event, not `input`, so a drag is one write rather than one per pixel.
